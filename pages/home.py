...
def _obtain_localized_fire_data(satellite_name):
    ...
    localized_eaton_subset = convert_timezone_for_dataset(
        eaton_subset, TIMEZONE)
    palisades_subset = subset_palisades_data(satellite_name)
    localized_palisades_subset = convert_timezone_for_dataset(
        palisades_subset, TIMEZONE)
    return {
...
def display_map(satellite_name, date_range, config):
    ...
    combined_data = pd.concat(
        [fire_subset_data["eaton"], fire_subset_data["palisades"]])
    gpd_combined_points = _convert_dataframe_to_geopandas_coordinates(
        combined_data)
...
def display_menu():
    ...
    selected_satellite = st.selectbox(
        "Choose satellite dataset", satellite_options)

    # update the custom config based on the selected heatmap display option
    # dict to map key to value
    key_value = {
        "FRP (MW)": "frp",
        "Brightness (Channel 21/22) [Kelvin]": "brightness",
        "Brightness (Channel 31) [Kelvin]": "bright_t31"}
    value_options = [
        "FRP (MW)",
        "Brightness (Channel 21/22) [Kelvin]",
        "Brightness (Channel 31) [Kelvin]"]
