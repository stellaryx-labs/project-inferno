# Utility services for processing NASA FIRMS fire data
...
def obtain_specific_fire_data(sensor_type):
    ...
    if sensor_type in fire_data:
        ...
        raise ValueError(
            f"Sensor type '{sensor_type}' not found in the fire data.")
...
def subset_palisades_data(satellite_name):
    ...
    palisades_fire_gdf = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(
            palisades_data['longitude'],
            palisades_data['latitude']),
        crs=palisades_shape_gdf.crs)
    palisades_fire_gdf = palisades_fire_gdf[palisades_fire_gdf.within(
        palisades_shape_gdf.unary_union)]
...
def subset_eaton_data(satellite_name):
    ...
    eaton_fires_gdf = gpd.GeoDataFrame(
        eaton_data,
        geometry=gpd.points_from_xy(
            eaton_data['longitude'],
            eaton_data['latitude']),
        crs=eaton_shape_gdf.crs)
    eaton_fires_gdf = eaton_fires_gdf[eaton_fires_gdf.within(
        eaton_shape_gdf.unary_union)]
